...
\begin{enumerate}[label=(\alph*)]
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item There exists a $\gamma > 0$ such that $A - \gamma I \succ 0$.
          \begin{mdframed}
            (Unsolved) We want to show that a $\gamma > 0$ exists such that
            \begin{align*}
              x^\T(A - \gamma I) x
              &= x^\T(Ax - \gamma Ix) \\
              &= x^\T Ax - \gamma x^\T x > 0
            \end{align*}
            for all $x \in \R^n$. Such a $\gamma$ would therefore have to satisfy
            \begin{align*}
              \gamma < \frac{x^\T Ax}{x^\T x} = \frac{|Ax|\cos \theta}{|x|}
            \end{align*}
            for all $x \in \R^n$, where $\theta$ is the angle between $x$ and
            $Ax$ and $|y|$ denotes the 2-norm.
          \end{mdframed}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \begin{mdframed}
      Viewing $a$ as a constant and $a^\T x$ as a function
      $f:\R^n \rightarrow \R$ with $f(x) = a^\T x$, the requested gradient is
