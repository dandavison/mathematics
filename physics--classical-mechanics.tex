...
\begin{mdframed}
  ...
  Show that for $n \in \N$, if $m_2/m_1 = 10^{2n}$ then the total number of collisions (including
  the left block bouncing off the wall) is equal to the integer formed from the first $n+1$ digits
  of the decimal expansion of $\pi$. So, for example:\\~\\
  \begin{tabular}{l|l}
    $m_2$ & #collisions \\
    \hline
    $m_1$        & 3 \\
  ...
  \textit{Suppose that an object with mass $m_1$ and velocity $u_1$ collides elastically with an
    ...
    \begin{align*}
      v_1 &= \frac{m_1v_1 - m_2v_1 + 2m_2v_2}{m_1 + m_2} \\
  ...
  sign.
\end{mdframed}\begin{comment}  % latex-focus
