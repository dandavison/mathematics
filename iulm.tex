...
If $|I| = 0$ then $I = \emptyset$ and the proposition is true since the LHS is
\begin{align*}
A \cap \bigcup_{i \in \emptyset} B_i = A \cap \emptyset = \emptyset,
\end{align*}
and the RHS is
\begin{align*}
  \bigcup_{i \in \emptyset} (A \cap B_i) = \emptyset.
\end{align*}

\textit{Remark}: we also know it holds for $|I| = 1$ (immediate) and $|I| = 2$
(a De Morgan's law).
...
\begin{align*}
  ...
  &= \(A \cap \bigcup_{i \in X} B_i\) \cup \(A \cap B_y\) ~~~\text{(a De Morgan's law)}\\
  &= \(\bigcup_{i \in X} (A \cap B_i)\) \cup \(A \cap B_y\) ~~~\text{(by assumption)}\\
  &= \(\bigcup_{i \in X \cup \{y\}} (A \cap B_i)\),
\end{align*}
as desired. This completes the proof by induction of the first proposition.
\end{mdframed}
