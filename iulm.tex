...
\begin{mdframed}
\begin{enumerate}[label=(\roman*)]
\item A function is a relation between two sets, which is a subset of the
  Cartesian product of the two sets. Therefore the number of $f: A \to B$ is
  $|\wp(A \times B)| = 2^{|A \times B|} = 2^{15}$.
\item Also $2^{15}$ by a similar argument.
\item An injective $f$
\end{enumerate}
\end{mdframed}
