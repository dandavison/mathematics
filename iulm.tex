...
\begin{mdframed}
\begin{enumerate}[label=(\roman*)]
\item The possible values for the domain of $f$ are $\wp(A)$. Consider the set
  of $f:A \to B$ with domain $A^*$ where $|A^*| = k$. For each element $a$ of
  the domain there are $|B| = 5$ possible choices for $f(a)$. Therefore there
  are $|B|^k$ such $f$ and the total number of $f$ is
  \begin{align*}
   \sum_{A^* \in \wp(A)} |B|^{|A^*|}
