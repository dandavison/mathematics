...
\begin{mdframed}
  ...
  \item The claim is that the union of two images is the image of the
    ...
    \textcolor{blue}{This is seeming a bit pedestrian! Are there more interesting proofs?}

    First we prove $f(A) \cup f(B) \subset f(A \cup B)$. If $A$ and $B$ are
    both empty then $\LHS = f(\emptyset) \cup f(\emptyset) = \emptyset$ and so
    the proposition is true. Alternatively, suppose $A$ is non-empty and let
    $x \in A$. Then $f(x) \in f(A)$ and so $f(x) \in \LHS$. Also
    $x \in A \cup B$ so $f(x) \in \RHS$. We could also have supposed $B$ is
    non-empty but due to the symmetries in LHS and RHS this would lead to the
    same conclusion.

    Second, we prove $f(A \cup B) \subset f(A) \cup f(B)$. If $A$ and $B$ are
    both empty then $\LHS = f(\emptyset) = \emptyset$ and so the proposition is
    true. Alternately assume $A$ is non-empty and let $x \in A$. Then $f(x)$ ...

  \item The claim is that the union of preimages is the preimage of the union.
  \end{enumerate}
\end{mdframed}
