...
\red{WRONG: it diverges at $p = 1$ according to the question, and to Mathematica:}
...
\item
  ...
  \begin{proof}
    ...
    % Note that $a_k = (-1)^{k-1}\frac{2\log \sqrt{k}}{\sqrt{k}} = (-1)^{k-1}ke^{-k/2}$.

    % Let $u = \sqrt{k}$. Then $\frac{|a_k|}{2} = \frac{\log u}{u} = ue^{-u}$.

    % Note that $|a_k| = ke^{-k^{1/2}}$, and that
    % \begin{align*}
    %   \int_1^\infty xe^{-x^{1/2}} \dx = x
    % \end{align*}
  \end{proof}
