...
\item Are the elements of a vector space always ``array-like'', allowing ``componentwise''
  ...
  tuples.) $\rightarrow$ Does every vector space have a basis $\rightarrow$ Axiom of Choice, Zorn's
  Lemma.
\item When is multiplication of functions defined to be composition versus componentwise
  multiplication? $(fg)(x) := f(g(x))$ vs $(fg)(x) := f(x)g(x)$. $\rightarrow$ Either might make
  sense.
\item In $\R^3$ a set of basis vectors have no natural order. But in e.g. the space of functions on
  ...
  identified there? $\rightarrow$ well, $R^n$ basis vectors do have an order.
\item Conrad says
  \begin{quote}
