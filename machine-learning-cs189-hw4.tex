...
\begin{mdframed}
  ...
  \end{align*}
Now $\w^\T\w = \sum_{j=1}^d w_j^2$, and $||\w||_1 = \sum_{j=1}^d |w_j|$, and
\begin{align*}
  y^\T\X\w = (\y^\T\X) \w = \sum_{j=1}^d \X_{*j}^\T ~\y w_j,
\end{align*}
so
\begin{align*}
  J(\w) = g(\y) + \sum_{j=1}^d f(\X_{*j}, w_j, y, \lambda),
\end{align*}
where
\begin{align*}
  g(\y) &= \y^\T\y ~~~~~~~~~~~~~~~~~~~~~~\text{and}\\
  f(\X_{*j}, w_j, \y, \lambda) &= nw_j^2 + \lambda|w_j| - \X_{*j}^\T ~\y w_j\\
                              &= |w_j|\(n|w_j| + \lambda\) - \X_{*j}^\T ~\y w_j.
\end{align*}
\end{mdframed}
