...
\begin{mdframed}
  My initial version was very verbose due to discomfort with matrix
  algebra. After looking at the lecture notes I see that it should be
  abbreviated, something like this:
...
\begin{align*}
  ...
  &= 2\lambda\w + \sum_i
    \frac{y_i}{s_i(\w)} \nabla s_i(\w) -
    \frac{1 - y_i}{1 - s_i(\w)} \nabla s_i(\w) \\
  % &= 2\lambda\w + \sum_i \nabla s_i(\w) \(\frac{y_i}{s_i(\w)} - \frac{1 - y_i}{1 - s_i(\w)}\) \\
  &= 2\lambda\w + \sum_i \nabla s_i(\w) \(
    \frac{y_i - s_i(\w)}
         {s_i(\w)(1 - s_i(\w))}
\) \\
