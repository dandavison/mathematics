...
\begin{align*}
  w^*_i =
  \begin{cases}
    \frac{ \lambda - 2\X_{*i}^\T ~\y }{2n}, &\lambda > 2\X_{*i}^\T ~\y \\
    0,                                     &\text{otherwise}.
  \end{cases}
\end{align*}
\end{mdframed}
...
\end{align*}
So the weight for the $i$-th feature is zero if and only if the $n$-vector
containing the $i$-th feature is orthogonal to the observed training values
$\y$.
\end{mdframed}
