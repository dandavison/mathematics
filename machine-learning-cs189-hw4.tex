...
\begin{align*}
  \nabla |\w|^2 = \nabla \sum_{j=1}^d w_j^2 = \cveccc{2w_1}{\vdots}{2w_d} = 2\w.
\end{align*}
\hlinee
...
\begin{align*}
  ...
  = (1 + e^{-\w\cdot\x_i}) \partiald{s_i(\w)}{w_j},
\end{align*}
...
\begin{align*}
  \partiald{\ln s_i(\w)}{w_j} =
  x_{ij}\frac{e^{-\w\cdot\x_i}}{1 + e^{-\w \cdot \x_i}} =
  x_{ij}(1 - s_i(\w)),
\end{align*}
...
\end{align*}
\hlinee
Now $\ln(1 - s_i(\w))$. We've already computed $(s_i(\w))'$, and $\frac{1}{1 - s_i(\w)} = \frac{1 + e^{-\w\cdot\x_i}}{e^{-\w\cdot\x_i}}$, so
\begin{align*}
  \partiald{\ln \(1 - s_i(\w)\)}{w_1}
  &= \frac{1 + e^{-\w\cdot\x_i}}{e^{-\w\cdot\x_i}}(-1) x_{ij}\frac{e^{-\w\cdot\x_i}}{(1 + e^{-\w \cdot \x_i})^2} \\
  &= -x_{ij}s_i(\w),
\end{align*}
and
\begin{align*}
...
\begin{align*}
  ...
  y_i     \nabla \ln s_i(\w) +
  (1-y_i) \nabla \ln(1 - s_i(\w)) \\
  &= 2\w + \sum_i y_i (1 - s_i(\w))\x_i - \sum_i (1 - y_i) s_i(\w)\x_i \\
  &=2\w + \y^\T(1 - \mat S)\X - (1 - \y)^\T\mat S\X,
\end{align*}
where $\mat S$ is a diagonal $(n \times n)$ matrix with $S_{ii} = s_i(\w)$.
\end{mdframed}
