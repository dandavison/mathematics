...
\item
  ...
  \begin{proof}~\\
    ...
    \red{The bounded functions $X \to Y$ are not a vector space; we have no addition in $Y$. So
      unlike the proof that the normed vector space of bounded real-valued functions is complete,
      we cannot argue that $f_n$ is bounded, therefore $f = f_n + (f - f_n)$ is bounded.}
