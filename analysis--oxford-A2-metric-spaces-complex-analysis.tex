...
\end{claim*}
\begin{proof}
  Let $x \in \bar{A \cup B}$. First suppose $x \in (A \cup B)'$ ... show that
  $x \in \bar A \cup \bar B$.

  Alternatively suppose that $x$ is an isolated point of $A \cup B$. WLOG suppose $x \in A$. Then
  $x \in \bar A$.
\end{proof}
...
% \begin{remark*}
%   Intuitively, does it seem like this should be true?
% \end{remark*}

% \begin{proof}
%   Let $f:\R \to \R$, and let $A \subseteq \R$.
...
\begin{proof}
  ...
  Let $B \subseteq M$. Recall that $B'$ denotes the set of limit points of $B$ and that
  $\bar B = B \cup B'$.

  Note that $f(\bar B) \subseteq \bar{f(B)} \iff f(B' \setminus B) \subseteq \bar{f(B)}$.

  (This is true because $\bar B = B \cup (B' \setminus B)$ and, since $f(B) \subseteq \bar{f(B)}$
  is true by definition, we only have to consider whether
  $f(B' \setminus B) \subseteq \bar{f(B)}$.)

  $\implies$\\
  Suppose $f$ is continuous and let $A \subseteq M$.

  By the above argument, for the forward implication it suffices to show that $f(m) \in \bar{f(A)}$
  where $m \in A' \setminus A$.

  Let $(x_n)$ be a sequence in $A$ that converges to $m$.

  Note that, since $f$ is continuous, $(f(x_n))$ converges to $f(m)$.

  Therefore $f(m)$ is a limit point of $f(A)$, as required.
  ...
  Note that this implies that $f(A' \setminus A) \subseteq \bar{f(A)}$.

  We need to show that $f$ is continuous. We will show this using the convergent sequences
