...
\item
  ...
  \begin{proof}~\\
    ...
    We want to show that $\mathring{B}(u, \epsilon) \cap U_0 \neq \emptyset$.

    Let $n \geq 2$ be the smallest integer greater than $1 + \log_2 \epsilon^\1$, so that
    $2^n > \frac{2}{\epsilon}$. Let $v \in M$ be such that $v_m =
    \begin{cases}
      u_m & m \neq n\\
      1 - u_m & m = n.
    \end{cases}$.  Then we have $v \in \mathring{B}(u, \epsilon)$, since
    $d(u, v) = 2^{-n} < \frac{\epsilon}{2}$. Therefore $U_0$ is open.

    Now consider $U_0^C = \{(x_n) \in M ~|~ x_0 = 1\}$. Note that the sequence
    $a = (1, 0, 0, \ldots) \in U_0^C$. Therefore $U_0^C$ is not open, since for all $\epsilon > 0$
    we have that $(0, 1, 1, \ldots) \in \mathring{B}(a, \epsilon) \cap U_0$.  \todo{Hmm. So one
      would think that $M$ is connected?}
  \end{proof}
...
\item
  ...
  \begin{proof}
    ...
    As above, let $n \geq 2$ be the smallest integer greater than $1 + \log_2 \epsilon^\1$, so that
    $2^{-n} < \frac{\epsilon}{2}$.

    Note that $\sum_{i=n}^m \frac{1}{2^i} < \frac{1}{2^{n-1}}$ for all $m \geq n$.

    OK, the details are annoying, but basically the set of sequences that are within $\epsilon/2$
    of $x$ are those that agree with $x$ over the first $n$ bit positions. Furthermore, the image
    under $f$ of that set of sequences is contained within $(f(x) - \epsilon, f(x) +
    \epsilon)$. Therefore for all $\epsilon > 0$ we have that $\delta=\epsilon/2$ works and hence
    $f$ is continuous.
  \end{proof}
\end{enumerate}
...
\item
  ...
  \end{claim*}
  \begin{proof}~\\
    \todo{Claim is unsurprising but I should finish the proof.}

    \red{Suppose $M$ is connected and empty. Then every integer-valued continuous function on $M$
      is constant and the claim is false. So we will suppose $M$ to be non-empty.}
