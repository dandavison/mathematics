...
\item
  ...
  \begin{proof}
    ...
    $L = \{u + \rho(v - u) ~|~ \rho \in [0, 1]\}$ be the straight line between $u$ and $v$, and let
    $<$ be a total ordering relation on $L$ defined by $l_1 < l_2 \iff |v - l_2| < |v - l_1|$. Let
    $S = \{l \in L ~|~ f(l) = f(u)\}$, let $T = \{l \in L ~|~ [u, l] \subseteq S\}$ and let
    $c = \sup T$.

    We claim that $f$ is not continuous at $c$. Indeed, suppose $c \in T$ so that $f(c) =
    f(u)$. Let $\epsilon = \frac{1}{2}$ and suppose there exists $\delta > 0$ such that
    $|l - \delta|$ is such that
