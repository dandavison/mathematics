...
\item
  ...
  \begin{proof}~\\
    ...
    For part (c), we need to show that there exists a limiting function $f \in \C(X, Y)$ such that
    for all $\epsilon > 0$ there exists an $N$ such that for all $n > N$ we have
    $\delta(f_n, f) < \epsilon$.

    We claim that $f(x) = L(x)$ is such a function.
    ...
    \red{It's a limit, but is it continuous?}
