...
\begin{theorem}\label{transformed-basis-is-a-basis}
  Let $U, V$ be vector spaces, let $f:U \to V$ be a linear map, and let $e_1, \ldots, e_n$ be a
  basis for $U$. Then $f(e_1), \ldots, f(e_n)$ is a basis for $V$.

  TODO: only if $f$ is an isomorphism? (i.e. injective, in which case it's surjective since any
  injective linear map is surjective (proof?)).
\end{theorem}
...
\begin{proof}We need to show that the $f(e_i)$ are linearly independent and spanning.
  ...
  \item {\bf Linear independence}\\
    Suppose $\sumin \lambda_1f(e_i) = 0$.

    Then $f\(\sumin \lambda_1e_i\) = 0$ since $f$ is linear.

    Therefore $\sumin \lambda_1e_i = 0$ since the preimage of 0 is $\{0\}$ under a linear map.
