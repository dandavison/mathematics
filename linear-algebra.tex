...
A ``solution'' is an assignment of values to the variables which makes all the
equations true.

In other words, we notice that the equations involve two variables, and
consider the set of 2-tuples $S = \{(x, y) ~|~ x, y \in \R\}$.

The set of solutions is the subset of $S$ for which the equations are all true.
...
The linear equations describe hyperplanes (lines, planes etc) in $S$.
...
Next, we rewrite the linear system as a matrix applied to a vector:
\begin{align*}
  \begin{array}{cc}
    \matMMxNN{1}{0}
