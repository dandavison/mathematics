...
\begin{enumerate}
\item We think of this matrix as 8 rows.
\item Focus on row 1: it contains the 3 peoples' ratings for one film.
\item The row is a vector in $\R^3$: it is $(3, 2, 5)$ with respect to the basis. What is the
  basis?
\item Writing the data with one column per person implicitly specified the basis.
\item The basis consists of 3 vectors, each corresponding to a single one of the 3 people.
\item The basis is $\{(1, 0, 0), (0, 1, 0), (0, 0, 1)\}$. For example, $(0, 1, 0)$ is the vector
  representation of Person 2.
\item Row 1 is a linear combination of the 3 people: it is
  $(3 \times \text{Person 1}) + (2 \times \text{Person 2}) + (5 \times \text{Person 3})$
\item Indeed, whenever we specify the coordinates of a vector in any vector space, we are specifying
  a linear combination of the basis vectors.
\item The row represents a person: an imaginary person that is a linear combination of the 3 real
  people.
\item This is despite the fact that the row is labeled ``Film 1'': it contains the 3 real peoples'
  ratings of Film 1, and these ratings specify a new, imaginary, linear-combination person.
\end{enumerate}
