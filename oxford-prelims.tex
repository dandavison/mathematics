...
contributing vectors from an existing spanning set. One implication is that
``dimension'' is well-defined: it is ... ?]
...
\begin{proof}~\\
  ...
  \begin{enumerate}[label=\roman*)]
  \item linearly independent vectors $v_1, \ldots, v_k$ exist, by hypothesis of
  ...
  \end{enumerate}
  ~\\
  Fact \ref{steinitz-span} implies that we can write $v_{j+1}$ as a unique linear combination:
  \begin{align*}
