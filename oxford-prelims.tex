...
\begin{proof}
  ...
  Let $u_1, \ldots, u_n \in T^\1(X)$ and let $T(u_i) = x_i \in X$ for
  $i=1,\ldots, n$. Consider the linear combination $\sum \lambda_iu_i$ for
  scalars $\lambda_i$. We have
  $T\Big(\sum \lambda_iu_i\Big) = \sum \lambda_ix_i \in X$ by the linearity of
  $T$ and by the fact that $X$ is a subspace. Therefore $T^\1(X)$ is closed
...
\begin{proof}
  Choose a basis $u_1, \ldots, u_n$ for $T^{-1}(X)$.
