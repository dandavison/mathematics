...
\begin{proof}
  ...
  $\sum_{i=1}^{r_{T^*}} \lambda_iu_{n_T+i} = \sum_{i=1}^{n_T} \alpha_iu_i$, for
  some scalars $\alpha_i, \ldots, \alpha_{r_{T^*}}$.

  Therefore $\sum_{i=1}^{n_T+r_{T^*}}\beta_iu_i = 0$, where each $\beta_i$ is
  either $\lambda_i$ or $-\alpha_i$. But since the $u_i$ are a basis of
