...
\item Define a cost function $l: \mc C \to [0, \infty]$
\item Define $\mu^*(E)$ to be the cost of the least-cost cover of $E$:
  \begin{align*}
    \mu^*(E) := \inf \Big\{\sum_{i=1}^\infty l(C_i) ~:~ E \subseteq \bigcup_{i=1}^\infty C_i\Big\}.
  \end{align*}
...
\begin{example}[Lebesgue measure on subsets of $\R$]
  ...
  \item Define $l(O)$ in the normal way (sum of $b_i - a_i$ over the disjoint open intervals).
