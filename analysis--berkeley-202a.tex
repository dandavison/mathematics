...
\begin{proof}
  ...
  Let $I = (a, b)$ and let $I_k = (a_k, b_k)$ for all $k$. There are two cases:
  \begin{enumerate}
  \item {\bf Finite cover}: if $\bigcup_{k=1}^n I_k \subseteq I$ (disjoint) then $\sum_{k=1}^n |I_k| \leq |I|$\\
    The claim is clearly true for $n = 1$. Assume it's true for $n = m-1$, i.e. that
    \begin{align*}
      \sum_{k=1}^{m-1} |I_k| \leq a_{m} - a.
    \end{align*}
    Label the intervals $I_1, \ldots, I_n$, sorted by their left endpoint in ascending order. Note that
    the $m$-th interval has length $b_m - a_m \leq b - a_m$. Thus we have
    \begin{align*}
      \sum_{k=1}^m |I_k|
      &= \sum_{k=1}^{m-1}|I_k| + |I_m| \\
      &\leq (a_m - a) + (b - a_m) \\
      &= b - a.
    \end{align*}
    Therefore it is true for all $n$ by induction.
  \item {\bf Infinite cover}: $\sum_{k=1}^\infty |I_k| \leq |I|$\\
    Since it is true for every finite subcover of size $n$, it is true for a cover comprising infinitely many
    intervals. (?)
  \end{enumerate}

  Second, we show that $\sum_k |I_k| \geq |I|$.
  There are two cases
  \begin{enumerate}
  \item {\bf Finite cover}: $\sum_{k=1}^n |I_k| \geq |I|$\\
    Again, label the intervals $I_1, \ldots, I_n$, sorted by their left endpoint in ascending order. It's clearly true
    Again, it's obvious for $n=1$. Suppose it's true for $n = m-1$, i.e. that
    \begin{align*}
      \sum_{k=1}^{m-1} |I_k| \geq (b - a)
    \end{align*}
