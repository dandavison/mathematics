...
\item We want a function that, in some appropriate sense, measures the {\it volume} of an arbitrary subset of
  $X$.

\item We're not going to get the ``sensible measure of volume​'' property out of nowhere: we're going to inject a
  pre-existing sensible measure of volume of tractable sets at a low level, and build on this.

\item That low-level ``sensible measure of volume​'' is, when we're working with $\R$, going to be the length of an
  interval: $|(a, b)| = b - a$.

\item Clearly we want our measure of volume to be additive over a finite collection of subsets. But we will also
...
\item So, more precisely, what we want is a {\it countably additive} set function (i.e. a \defn{measure}) defined on a large collection (as
  large as possible) of subsets of $X$, that is a ``sensible measure of volume​'' of those subsets.
...
\item A theorem tells us a way to make a {\it countably sub-additive} set function (i.e. an \defn{outer measure}) defined on {\it all} subsets:
  \begin{enumerate}
  \item Let $E$ be an arbitrary subset of $X$ that we want to measure.
  \item Now, restrict attention to the collection of ``low-level​'' subsets of $X$ for which we have the pre-existing
    sensible measure of volume. In $\R$, these are open intervals, or perhaps half-open intervals \red{open sets?}.
  \item Sometimes, one of these low-level subsets will cover $E$. But if $E$ is not a simple interval, we will
    approximate the volume of $E$ better with a collection of low-level subsets whose union covers $E$, while
  ...
  \item Note that the covering collection is built out of the low-level subsets, so we can assign a sensible
    measure of volume to the covering collection: in $\R$, it is just the sum of lengths of the intervals involved.
...
\item We will call our outer measure $\mu^*$. Clearly it is a reasonable measure of volume for some sets.

\item Recall that it is defined on {\it all} subsets of $X$ (its definition involved our restricted collection of
  ``low-level​'' subsets, but the resulting procedure can be applied to any subset).

\item Now, it's nice that it is defined on all subsets of $X$, and it does have some sensible properties such as
...
\item However, there's a larger $\sigma$-algebra we can restrict to: the \defn{Lebesgue} $\sigma$-algebra. This is the class
  of $\mu^*$-measurable sets. A set $A \subset X$ is $\mu^*$-measurable if finite additivity holds between $A$
  and every other subset of $X$, that is $\mu^*(A \cap E) + \mu^*(A \cap E^c) = \mu^*(A)$ for
  all $E \subset X$.

\item The Borel $\sigma$-algebra is contained within the Lebesgue $\sigma$-algebra. Restricting $\mu^*$ to either
  gives us what we want: countable additivity.
