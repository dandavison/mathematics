...
\item
  ...
  \begin{proof}
    \begin{enumerate}
    \item {\bf Reflexivity}\\
      Let $[x, x] = \{x\}$ be a closed interval containing a single point. We see that $\sim$ is not an
      equivalence relation since $x \not \sim x$ for all $x \in \R \setminus O$.
...
\item
  \begin{proof}
    $O$ is open. By definition, every point in $O$ has a neighborhood that is contained within $O$.
