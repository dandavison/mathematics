...
\begin{enumerate}
\item What's the difference between iff and two implications?
  \begin{enumerate}
  \item \item {\bf Comparison test; simple form}: If $0 \leq a_k \leq Cb_k$ then:
    \begin{itemize}
    \item $\sum b_k$ converges $\implies \sum a_k$ converges.
    \item $\sum a_k$ diverges $\implies \sum b_k$ diverges.
    \end{itemize}
  \item {\bf Comparison test; limit form}: If $a_k, b_k > 0$ for all $k$ and
    $\frac{a_k}{b_k} \to L$ then $\sum a_k$ converges iff $\sum b_k$ converges.
  \end{enumerate}
\item An example of a simple function is $f:\{0\} \to \{0\}$. What are the simplest functions for
  which limit, continuity and differentiability are defined? And interesting/meaningful?

\item I think the following is true: If $f$ is continuous and injective, and if $f(x_n) \to b$ then $x_n \to f^{-1}(b)$.

\item Let $a_k = \frac{p(k)}{q(k)}$ where $p(k)$ is a degree 2 polynomial and $q(k)$ is a degree 3
  polynomial. Is it correct to say that $a_k = O(k^{-1})$? If not is there a similar statement that
  is correct?q
\end{enumerate}
