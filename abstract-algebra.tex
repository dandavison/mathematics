...
\item So now you have a new group $G/{\sim}$ containing equivalence classes. There are four
  ...
  \item Somewhat obvious: this one-to-one correspondence is actually an isomorphism.
    ...
    This is fairly obvious: in $G$ we have collapsed elements that map to the same value in $H$,
    into a single element, associated that new element with the value in $H$, and declared that,
    when it comes to the binary group operation, its behavior is inherited from the elements in $G$
    that were collapsed to form it. So if we had a non-injective homomorphism to start off with,
    it's clear that we now have an injective homomorphism, because we have coalesced the elements
    on the left-hand side that mapped to the same element; i.e. there's now an isomorphism between
    the equivalence classes and the image of $f$.

    Question: is this obvious? Clearly it comes about because the group operation on the
    equivalence classes was defined by reference to the original group operation on $G$, and by
    reference to $f$. However, the group operation on $H$ was never used directly, and yet there is
    an isomorphism $G/{\sim} \cong \Im f \subseteq H$.
  \end{enumerate}
\end{enumerate}
