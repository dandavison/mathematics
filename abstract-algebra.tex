...
\url{https://gowers.wordpress.com/2011/10/16/permutations/}

Consider the set $\{1, 2, \ldots, 6\}$. What exactly is the permutation that in cycle notation is
written $(124)$?
...
When visualizing permutations, this corresponds to ``$1$ becomes $2$, and $2$ becomes $4$, and $4$
becomes $1$​'', wherever those numbers happen to be.



\begin{enumerate}
\item {\bf cycle the elements $1, 2$ and $4$}\\
  Arrange the numbers $1, 2, \ldots, n$ in a line. Now .
\item {\bf cycle the elements in the places $1, 2$ and $4$}
...
\begin{definition}
  A group action of a group $G$ on a set $A$ is a function $G \times A \to A$. We write this
  as $ga$. It satisfies:
  \begin{enumerate}
  \item $g_1(g_2a) = (g_1g_2)a$
  \item $1a = a$
  \end{enumerate}
\end{definition}

The idea is that the group is abtract, but the set $A$ might be very concrete: e.g. $A$ could be
the physical object or shape.


Orbit-stabilizer theorem: consider the 24 rigid motions of a cube. The cube is oriented so that the
red face is facing you. 4 of the rigid motions send the red face to itself. Therefore 4 of them
send the red face to the blue face (pick any rigid motion that brings the blue face to where the
red face was; now compose that with the identity and the 3 rotations that send blue to blue).

\begin{mdframed}
\includegraphics[width=400pt]{img/abstract-algebra--group-actions-1cd6.pngx}
