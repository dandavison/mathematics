...
So far everything has been straightforward, but here is the first subtle point:
...
\item So now you have a new group $G/{\sim}$ containing equivalence classes. There are four
  ...
  \item Obvious: All information about the original multiplicative structure on $G$ is preserved in
    the multiplicative structure on $G/{\sim}$. This is because we decided that the group operation
    on the equivalence classes would be inherited from $G$.
  ...
  \item Less obvious: this one-to-one correspondence is actually an isomorphism.
