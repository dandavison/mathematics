...
\begin{proof}
  ...
  % Since $\dy = \dt ~ \dot{y}(t)$, we can write the integral as
  % \begin{align*}
  %   S[y] = \int_0^{y_1} \frac{1}{\dot{y}}\dy.
  % \end{align*}
  % \begin{mdframed}
  %   OK, so can we just apply Euler-Lagrange now to solve it? Let
  %   $f(x, y, y') = \frac{1}{\dot{y}}$, and we have
  %   \begin{align*}
  %     \ddx \pdfdyp - \pdfdy = 0.
  %   \end{align*}
  %   I think the problem is we don't know how to differentiate $\frac{1}{\ydot}$ w.r.t. $y$ or $y'$.
  % \end{mdframed}

  % So that doesn't immediately solve the problem because we don't know what $\ydot$ is. So what \textit{do}
  % we know?
