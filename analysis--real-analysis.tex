...
\begin{theorem*}
  $\frac{n^k}{c^n} \to 0$ as $n \to \infinity$ for $k > 1$, $c > 1$.
\end{theorem*}
\begin{proof}
  Let $c = 1 + b$. Then
  \begin{align*}
...
\begin{definition*}~\\
  ...
  {\bf Note}: The following definition of ``series'' is non-standard and only half serious. In
  ...
  roughly speaking, a series is a ``summation of a sequence''; the limit in question may or may not
  exist.
  ...
  The sequence of partial sums $(s_n)_{n \geq 1}$ is the {\bf series}.

  If $\lim_{n \to \infty} s_n$ exists then the {\bf series converges}.

  The notation $\sum_{k \geq 1} a_k$ is used to mean the series $(s_n)_{n \geq 1}$.
\end{definition*}

\begin{intuition*}~\\
  A sequence is a sequence of ``steps''.

  A series is the sequence of locations.
\end{intuition*}
...
\begin{theorem*}\hspace{0pt}
  ...
  \item {\bf Comparison test; simple form}: If $0 \leq a_k \leq Cb_k$ and the $\sum b_k$ converges
    then $\sum a_k$ converges.
...
So what we want is to show that $f(x) = \sum_{n=1}^\infty \frac{x^{n+1}}{n+1} = -\log(1 - x)$ is
continuous at the boundary point $x = -1$. I.e. we want to show that
\begin{align*}
  \lim_{x\to -1} \sum_{n=1}^\infty \frac{x^n}{n} = -\log(2).
\end{align*}
