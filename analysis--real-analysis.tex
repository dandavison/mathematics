...
\begin{theorem*}[Connection between sequences and functions]
  ...
  \item For every sequence $(x_n)$ such that $x_n \neq a$
    \begin{align*}
      \(x_n \to a ~\text{as}~ n \to \infty\) \implies \(f(x_n) \to f(a) ~\text{as}~ n \to \infty\).
    \end{align*}
    \begin{align*}
      \(\limn x_n = a\) \implies \(\limn f(x_n) = f(a)\).
    \end{align*}
    \begin{align*}
      \Lim{x_n}{a}{n}{\infty} \implies \Lim{f(x_n)}{f(a)}{n}{\infty}
    \end{align*}
  \end{enumerate}
\end{theorem*}
...
\begin{proof}
  ...
  Then there exists a sequence $(q_n)$ such that $q_n \in \Q$, $q_n \neq q$ and
  $\Lim{q_n}{q}{n}{\infty}$. Therefore $\Limp{f(x)}{x}{q} = $
...
\begin{definition*}[continuity]
  $f$ is continuous at $a$ if $\Lim{f(x)}{f(a)}{x}{a}$.
\end{definition*}

\begin{definition*}[limit]
  $\Lim{f(x)}{f(a)}{x}{a}$ if for all $\epsilon > 0$ there exists $\delta > 0$ such that
