...
We want to compute the gradient vector, i.e. partials $\partiald{L}{V_{hj}}$ and
$\partiald{L}{w_k}$.

\begin{align*}
  \partiald{L}{W_{kh}}
  &= \partiald{}{W_{kh}} \sum_k y_k \log(\hat y_k) + (1 - y_k) \log( 1 - \hat y_k) \\
  &= \partiald{}{W_{kh}} \sum_k y_k \log(\sigma(\mat W \z)) + (1 - y_k) \log( 1 - \sigma(\mat W \z)) \\
  &= \sum_k
     \frac{y_k    }{    \sigma(\mat W \z)}\partiald{\sigma(\mat W \z)}{W_{kh}} -
     \frac{1 - y_k}{1 - \sigma(\mat W \z)}\partiald{\sigma(\mat W \z)}{W_{kh}} \\
  &= \sum_k \partiald{\sigma(\mat W \z)}{W_{kh}} \Big(
     \frac{y_k    }{    \sigma(\mat W \z)} -
     \frac{1 - y_k}{1 - \sigma(\mat W \z)}   \Big) \\
  &= \sum_k \sigma(\mat W \z)(1 - \sigma(\mat W \z)) \partiald{\mat W \z}{W_{kh}} \Big(
     \frac{y_k    }{    \sigma(\mat W \z)} -
     \frac{1 - y_k}{1 - \sigma(\mat W \z)}   \Big) \\
\end{align*}

First consider a parameter $w_k$ in the final map $\vec h \to \hat y$: it has some
associated entry $\partiald{L}{w_k}$ in the gradient vector.
