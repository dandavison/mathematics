...
% 1. I *think* you mean to say that \int g~d\lambda = \int fg~d\mu but it's not quite clear. (-1)
% Aidan Backus, Oct 28 at 8am
% 4. It does not follow from the assumption that f is measurable and finite that there is an N with f \leq N. (Take f(x) = 1/x.) However, this can be repaired -- notice that wlog you may assume that f is simple, and therefore bounded. (-5)
% Aidan Backus, Oct 29 at 3:08pm
% 5. Why is this c = sup{|f| } finite? (it doesn't have to be, so this won't work). Your first lemma is true, though. Consider f defined on [0,1], where, f(1/n) = n for all n natural numbers, f(x) = 0 otherwise. Then I is empty, and f is integrable, but c is still infinity. Delete comment: 5. Why is this c = sup{|f| } finite? (it doesn't have to be, so this won't work). Your first lemma is true, though. Consider f defined on [0,1], where, f(1/n) = n for all n natural numbers, f(x) = 0 otherwise. Then I is empty, and f is integrable, but c is still infinity. (-7)
% Ian Francis, Oct 31 at 12:04pm
% 1. 9 3. 10 4. 5 5. 3
% Ian Francis, Oct 31 at 12:20pm
