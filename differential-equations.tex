...
so
\begin{align}
|e_n(t)| \leq L^nM\frac{h^{n+1}}{(n+1)!} \label{picard-convergence-induction}
\end{align}
...
This is exactly the same situation as \eqref{picard-convergence-induction} with
$d_n(t)$ substituted for $e_n(t)$, therefore $d_n(t) \to 0$ as $n \to \infty$
for all $t$, proving that if $Y$ is a solution, then $y_\infty = Y$. This
completes the proof of Picard's existence theorem. \qed
