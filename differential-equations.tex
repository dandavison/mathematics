...
\begin{align*}
  ...
  |e_2(t)| & \leq L\Big|\int_{t_0}^t \Big|y_1(\tau) - y_0(\tau)\Big| \d\tau \Bigg|\\
           ...
           & \leq LM|t - t_0|^2 \leq LMh^2\\
  |e_3(t)| & \leq L\Big|\int_{t_0}^t \Big|y_2(\tau) - y_1(\tau)\Big| \d\tau \Bigg|\\
           &    = L\Big|\int_{t_0}^t |e_2(\tau)| \d\tau \Bigg|\\
           & \leq L^2M|t - t_0|^3  \leq L^2Mh^3\\
\end{align*}
So it seems that $|e_n(t)| \leq L^{n-1}Mh^n$. To prove this, note that we know
it is true of $e_1$. So suppose it is true of $e_n$. The next term is
\begin{align*}
...
$\sum_{n=0}^\infty W_n$ converges. Our current proposal for the terms is
\begin{align*}
  W_n = L^{n-1}Mh^n.
\end{align*}

\red{TODO: Other proofs have a factorial term $n!$ as the denominator of these
  bounds (e.g. see P. J. Collins textbook below). Understand the origin of this
  factorial term and complete the proof that the series converges by
  Weierstrass M-test.}

\begin{mdframed}
\includegraphics[width=400pt]{img/z1.png}
\end{mdframed}
