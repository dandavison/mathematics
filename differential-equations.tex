...
\begin{align*}
  ...
  |e_2(t)| &    = L\Bigg|\int_{t_0}^t |e_1(\tau)| \d\tau\Bigg|  ~~~~~~~~~~~~~~~~~~~~~~~~\text{(by assumption that $v$ is Lipschitz in $y$)}\\
           ...
           &    = L^2M\frac{|t - t_0|^3}{3!}.
\end{align*}
So it seems that $|e_n(t)| \leq L^nM\frac{h^{n+1}}{(n+1)!}$. To prove this, note that we know
it is true of $e_0$. So suppose it is true of $e_n$. Then the next term is
\begin{align*}
...
\end{align*}
so $|e_n(t)| \leq L^nM\frac{h^{n+1}}{(n+1)!}$ for all $n \geq 0$ by induction.

Recall that our aim is to show that the series
$\sum_{n=0}^\infty W_n = L^nM\frac{h^{n+1}}{(n+1)!}$ converges. According to the
...
\begin{align*}
  |d_0(t)| &\leq M|t - t_0|\\
  |d_1(t)| &\leq \Bigg|L\int_{t_0}^t \Big|d_0(\tau)\Big| \d\tau\Bigg|\\
           &\leq \Bigg|L\int_{t_0}^t M|\tau - t_0| \d\tau\Bigg|\\
           &   = LM\frac{|\tau - t_0|^2}{2}\Bigg|_{t_0}^t\\
           &   = LM\frac{|t - t_0|^2}{2}\\
  |d_2(t)| &\leq \Bigg|L\int_{t_0}^t \Big|d_1(\tau)\Big| \d\tau\Bigg|\\
           &\leq \Bigg|L^2M\int_{t_0}^t \frac{|\tau - t_0|^2}{2} \d\tau\Bigg|\\
           &   = L^2M \frac{|t - t_0|^3}{3!}.
\end{align*}
\newpage
So it seems that
\begin{align*}
  |d_n(t)| \leq L^nM \frac{|t - t_0|^{n+1}}{(n+1)!^{~~~~}}
\end{align*}
This is true for $n=0$. For induction, suppose that it is true for $n$. Then
\begin{align*}
  |d_{n+1}(t)| &\leq L\Bigg|\int_{t_0}^t \Big|d_{n-1}(\tau)\Big| \d\tau\Bigg|\\
              &   = L^{n+1}M \Bigg|\int_{t_0}^t \Big|\frac{|t - t_0|^{n+1}}{(n+1)!^{~~~~}}\Big| \d\tau\Bigg|\\
              &   = L^{n+1}M \frac{|t - t_0|^{n+2}}{(n+2)!^{~~~~}}\\
              &\leq L^{n+1}M \frac{h^{n+2}}{(n+2)!^{~~~~}}
\end{align*}

We've seen already \eqref{picard-ratio-test} that the limit of this sequence is
zero, by the Ratio Test.

Therefore $d_n(t) \to 0$ as $n \to \infty$ for all $t$, proving that if $Y$ is
a solution, then $y_\infty = Y$. This completes the proof of Picard's existence
theorem. \qed
