...
\begin{proof}
  ...
  contains $n$ copies of each digit. A combinatorial argument show that this probability is equal to
  \begin{align*}
    \mu(A_n) = P(A_n)
  ...
  \mu(A_n)
    ...
    &< \frac{\prod_{i=1}^{10} (in)!}{10^{10n}} \\
    &< \frac{((10n)!)^{10}}{10^{10n}}
  \end{align*}
  \red{TODO}
  Therefore $\sum_{n=1}^\infty \mu(A_n) < \infty$.

  \begin{minted}{wolfram} :results latex
    (((10n)!)^10) / (10^(10n)) // FullSimplify
  \end{minted}
...
\begin{align*}
ToExpression::sntx: Invalid syntax in or before "\frac{((10n)!)^10}{10^(10n)} // TeXForm // ToString".
                                                 ^
\end{align*}
...
\end{align*}
  ...
  \begin{minted}{wolfram} :results latex
    Factorial[i*n] / (Factorial[n]*Factorial[i*n - n]) // FullSimplify
  \end{minted}
