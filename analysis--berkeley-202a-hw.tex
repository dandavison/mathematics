...
\item
  \begin{proof}
    ...
    % Since $O \subseteq \R$ is open, $\inf O$ and $\sup O$ exist.

    % $O$ is an open subset of $\R$. By definition, every point in $O$ has a neighborhood that is contained
    % within $O$. Since we are working in $\R$, every neighborhood is an interval.

    % Let $x \in O$. Then some interval containing $x$ is a subset of $O$. So $O$ contains no isolated points.

    % So $O$ is a union of intervals. Since we may merge intervals, $O$ is a union of disjoint intervals.
    % Furthermore, if any interval is closed, then it contains one of its endpoints. For any such endpoint $x$
    % there is an interval containing $x$ which we may merge with the first interval. We may continue merging
    % like this until the resulting interval is open. Thus $O$ is a union of disjoint open intervals.

    % Let $i, j \in A$ and let $I_i \subseteq \R$ be disjoint from $I_j \subseteq \R$ for $i \neq j$.
    % Then $O = \bigcup_{i \in A} I_i$. We need to show that the index set $A$ is countable.
