...
\item~\\
  ...
  \item
    \begin{proof}
      Let $\mc F = \bigcup_{n=1}^\infty \mc F_n$. We must show that
      \begin{enumerate}
      \item $\Omega \in \mc F$ \\
        {\bf Proof:} $\mc F_1$ is a algebra, therefore $\Omega \in \mc F_1$, therefore $\Omega \in \mc F$.
      \item If $A \in \mc F$ then $A^c \in \mc F$ \\
        {\bf Proof:} If $A \in \mc F$ then $A \in F_n$ for some $n$. Therefore $A^c \in \mc F_n$. Therefore $A^c \in \mc F$.
      \item If $A, B \in \mc F$ then $A \cup B \in \mc F$ \\
        {\bf Proof:} If $A, B \in \mc F$ then for some $m$ and $n$ we have $A \in \mc F_m$ and $B \in \mc F_n$.
        Suppose $m = n$. Then $A \cup B \in \mc F_m$, therefore $A \cup B \in \mc F$. Alternatively
        suppose $m \neq n$. Then either $\mc F_m \subset \mc F_n$ or $\mc F_n \subset \mc F_m$. Suppose without
        loss of generality that $\mc F_m \subset \mc F_n$. Then $A, B \in \mc F_n$,
        therefore $A \cup B \in \mc F_n$, therefore $A \cup B \in \mc F$.
      \end{enumerate}
    \end{proof}
  \item
    \begin{proof}
      Let $\Omega = (0, 1]$, let $\mc A_n$ be the set of rank-$n$ dyadic intervals in $[0, 1]$ and
      define $\mc F_n = \sigma(\mc A_n)$, the $\sigma$-algebra generated by $\mc F_n$. Then for example we have
      \begin{align*}
        \mc F_1 &= \Big\{\emptyset,
                         (0, .5], (.5, 1],
                         (0, 1]
        ...
        %             \Big\}
      \end{align*}
      Note however that $(1 - 2^{-n}, 1] \in \mc F_n$ and that $\bigcap_{n=1}^\infty (1 - 2^{-n}, 1] = \{1\}$.
      Therefore if $\bigcup_{n=1}^\infty \mc F_n$ is a $\sigma$-algebra
      then $\{1\} \in \bigcup_{n=1}^\infty \mc F_n$.

      But $\{1\}$ is not a dyadic interval, therefore there is no $n$ for which $\{1\} \in \mc A_n$.
      Furthermore there is no $n$ for which $\{1\} \in \sigma(\mc A_n)$ (justification below).

      Therefore $\{1\} \notin \bigcup_{n=1}^\infty \mc F_n$ and so $\bigcup_{n=1}^\infty \mc F_n$ is not a $\sigma$-algebra.

      ~\\
      \textbf{Justification that there is no $n$ for which $\{1\} \in \sigma(\mc A_n)$:}

      By definition, $\sigma(\mc A_n)$ is the intersection of all $\sigma$-algebras that include $\mc A_n$.
      Suppose $\{1\} \in \sigma(\mc A_n)$. Now form a new class of sets $\sigma^*(\mc A_n)$ by removing
      from $\sigma(\mc A_n)$ every set that contains $1$ as an isolated point, and its complement. We claim
      that $\sigma^*(\mc A_n)$ is a $\sigma$-algebra. Note that none of the removed sets were in $\mc A_n$
      (since they are not dyadic intervals). But
      then $\mc A_n \subseteq \sigma^*(\mc A_n) \subset \sigma(\mc A_n)$ which contradicts the definition
      of $\sigma(\mc A_n)$. Therefore $\{1\} \notin \sigma(\mc A_n)$.
    \end{proof}
  \end{enumerate}
...
\item~\\
  ...
  \end{mdframed}
  Let $\mc F_1, \mc F_2, \ldots$ be the collection of all fields in $\Omega$ for which $\mc A \subset \mc F_n$,
  so that $f(\mc A) = \bigcap_{n}\mc F_n$.
  ...
  \item
    ...
    \begin{proof}
      We must show that
      \begin{enumerate}
      \item $\Omega \in f(\mc A)$\\
        {\bf Proof:} $\Omega \in \mc F_n$ for all $n$, therefore $\Omega \in \bigcap_{n}\mc F_n = f(\mc A)$.

      \item If $X \in f(\mc A)$ then $X^c \in f(\mc A)$\\
        {\bf Proof:} If $X \in f(\mc A)$ then $X \in \mc F_n$ for all $n$, therefore $X^c \in \mc F_n$ for all $n$, therefore $X^c \in \bigcap_{n}\mc F_n = f(\mc A)$.

      \item If $X, Y \in f(\mc A)$ then $X \cup Y \in f(\mc A)$\\
        {\bf Proof:} If $X, Y \in f(\mc A)$ then $X, Y \in \mc F_n$ for all $n$, therefore $X \cup Y \in \mc F_n$ for all $n$, therefore $X \cup Y \in f(\mc A)$.
      \end{enumerate}
    \end{proof}
    ...
    \end{claim*}
    \begin{proof}
      Let $X \in \mc A$. Then $X \in \mc F_n$ for all $n$. Therefore $X \in \bigcap_{n}\mc F_n = f(\mc A)$. Therefore $\mc A \subset f(\mc A)$.
    \end{proof}

    \begin{claim*}
      $f(\mc A)$ is minimal in the sense that if $\mc G$ is a field and $\mc A \subset \mc G$, then $f(\mc A) \subset \mc G$.
    \end{claim*}
    \begin{proof}
      If $\mc G$ is a field with $\mc A \subset \mc G$ then $\mc G \in \{\mc F_1, \mc F_2, \ldots\}$, therefore $\mc G \supset \bigcap_{n}\mc F_n = f(\mc A)$.
    \end{proof}
  ...
  \item
    ...
    \begin{claim*}
      Every element of $f(\mc A)$ can be written in the form
      \begin{align*}
        \bigcup_{i=1}^m \bigcap_{j=1}^{n_i} A_{ij},
      \end{align*}
      where ...
    \end{claim*}

    \begin{proof}
      Let $\mc A = \{A_1, A_2, \ldots\}$and let $X \in f(\mc A)$.
      ...
      We proceed by induction. Since $f(\mc A)$ is an algebra, but not in general a $\sigma$-algebra, it can be
      ...
      Then it is possible to construct $X$ from the elements of $\mc A$, using complementation, finite unions
